...
class Scoop{
    ...
    async search(word="", page = 1, size = 100){
        ...
        for (const item of data.value) {
            let authorName = item.Metadata.AuthorName;
            if (authorName === "github-actions[bot]" && item.Homepage.includes("github.com")){
                authorName = item.Homepage.replace("https://github.com/", "");
            }
            items.push({
                title: item.Name + " - v" + item.Version + " - #" + authorName,
                description: item.Description,
                icon: item.Homepage ? 'https://favicon.yandex.net/favicon/' + this.#domain(item.Homepage) + '?size=32' : './logo.png',
                info: item
            })
        }
